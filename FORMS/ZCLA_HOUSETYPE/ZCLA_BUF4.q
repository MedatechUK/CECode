...
*/
SELECT ZCLA_FIXUPLIFT + ZCLA_BRUPLIFT  +  MOD_HT + MOD_ST
,   :POINTS * (1 + ((ZCLA_FIXUPLIFT + ZCLA_BRUPLIFT  +  MOD_HT + MOD_ST) / 100 ))
...
,    ZCLA_TOTCOST = (( :CASHVALUE * :ZCLA_LABOURPOINTS) + :TRAVEL )
+                   (:PARTSUM  )
+                    :TOTML
...
SELECT ZCLA_HOUSETYPEFIX.FIXID
,      :ZCLA_TOTCOST * (ZCLA_ELEMENTFIX.SPLIT / 100) * ZCLA_ELTYPE.MARKUP
FROM   ZCLA_ELTYPE
,      ZCLA_PLOTELEMENT
,      ZCLA_HOUSETYPEFIX
,      ZCLA_HOUSETYPE
...
AND    ZCLA_HOUSETYPEFIX.FIXID = ZCLA_ELEMENTFIX.FIXID 
